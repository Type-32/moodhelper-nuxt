...
const activeResponse = ref<string>(), awaitingResponse = ref<boolean>(false), generatingResponse = ref<boolean>(false), errorOccurred = ref<boolean>(false)
    ...
    <div class="flex flex-col min-h-screen items-center align-top justify-start max-tablet:px-8">
            ...
            <div class="navbar w-full gap-2 group z-10">
                <div class="flex transition duration-300 group-hover:backdrop-blur-lg group-hover:translate-y-1 bg-base-100/70 backdrop-blur-sm rounded-box shadow-lg w-full p-2 gap-2 z-10">
                    ...
                    <div class="join flex flex-row flex-grow">
                        ...
                        <div class="join-item">
                            <button class="btn btn-neutral join-item h-full" :disabled="awaitingResponse || generatingResponse || input == '' || errorOccurred" @click="seek()">{{ $t('navbar.button.text') }}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full flex-grow flex-col flex" v-if="messages.length == 0">
            <div class="flex flex-col justify-center items-center flex-grow gap-5">
                ...
                <span class="text-base-content/70 text-center">{{ $t('main.check_docs.1') }} <a href="/docs" class="link link-hover font-bold">{{ $t('main.check_docs.2') }}</a></span>
            </div>
        </div>
