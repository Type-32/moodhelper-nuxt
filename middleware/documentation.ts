export default defineNuxtRouteMiddleware((to, from) => {
    ...
    const userLocale = typeof window !== 'undefined' ? localStorage.getItem('page-locale') || 'en' : 'en';

    // Check if the path is exactly '/docs' or '/docs/en'
    if (basePath === 'docs' && (secondPath === undefined || thirdPath === undefined)) {
        // Redirect to '/docs/user-locale/getting-started'
        return navigateTo(`/docs/${userLocale}/getting-started`);
    }
});
