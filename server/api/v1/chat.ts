...
export default defineEventHandler(async (event) => {
  ...
  let messages: Message[] = [
    {
      role: "system",
      content:
        "You instructions in response to the user's prompt (format your response in markdown for better readability for the user, and also respond in the user's prompted language.):\n" +
  ...
  const openai = new OpenAI({
    ...
    defaultHeaders:
      config.public.aiEndpoint.indexOf("openrouter.ai") != -1
        ? {
  ...
  try {
    ...
    throw createError({
      ...
      message:
        error.message ||
        "An unknown error occurred while fetching stream response",
    });
  }
});
